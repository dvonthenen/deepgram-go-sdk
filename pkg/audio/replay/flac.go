...
func NewFlac(opts ReplayOptions) (*FlacClient, error) {
...
func (c *FlacClient) Start() error {
	reader, err := flac.New(c.file)
	if err != nil {
...
func (c *FlacClient) Read() ([]byte, error) {
...
func (c *FlacClient) Stream(w io.Writer) error {
	for {
		select {
		case <-c.stopChan:
